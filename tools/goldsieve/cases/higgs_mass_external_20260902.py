# -*- coding: utf-8 -*-
"""Внешняя сверка формулы массы бозона Хиггса.
...
def _observed() -> float:
    marker = r"125.25 | $(5, 3, 0, 4, -2)$ | 125.226 |"
    with open(SOURCE, encoding="utf-8") as handle:
        for line in handle:
            if marker in line:
                return 125.25
    raise AssertionError("строка корпуса с массой бозона Хиггса не найдена")


def _reference() -> float:
    return 5.0 * 3.0**3 * PHI**4 * math.e**-2


def _reference_alt() -> float:
    return math.exp(
        math.log(5.0) + 3.0 * math.log(3.0) + 4.0 * math.log(PHI) - 2.0
    )
...
def _external_target() -> dict:
    return {
        ...
        "название": "масса бозона Хиггса по PDG 2024",
    }
...
CLAIMS = [
    Claim(
        name="Формула массы бозона Хиггса против внешней оценки PDG 2024",
        source="docs/docs/math-foundations/sacred-formulas.md:43",
        ...
        claim_family="higgs_mass",
        observable="масса бозона Хиггса (ГэВ)",
        measurement_source="PDG 2024, таблица калибровочных бозонов и бозона Хиггса",
        uncertainty_type="both",
        expected_effect_sigma=0.236,
        resolution_sigma=1.0,
        novelty_key="boson:higgs_mass:external:v1",
        information_class="novelty",
        purpose="audit",
        models=["формула Trinity", "масса бозона Хиггса PDG 2024"],
        independent_of=[
            "zeta",
            "BBLM",
            "CKM",
        ...
        notes=(
            "125.25 прочитано из строки корпуса; внешняя цель "
            "125.20 ± 0.11 ГэВ взята из таблицы PDG 2024. "
