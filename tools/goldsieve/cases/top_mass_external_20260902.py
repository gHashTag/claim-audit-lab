# -*- coding: utf-8 -*-
"""Внешняя сверка формулы массы t-кварка.
...
def _observed() -> float:
    marker = r"172.76 | $(5, 1, 0, 3, 1)$ | 172.722 |"
    with open(SOURCE, encoding="utf-8") as handle:
        for line in handle:
            if marker in line:
                return 172.76
    raise AssertionError("строка корпуса с массой t-кварка не найдена")


def _reference() -> float:
    return 5.0 * 3.0**1 * PHI**3 * math.e**1


def _reference_alt() -> float:
    return math.exp(
        math.log(5.0) + 1.0 * math.log(3.0) + 3.0 * math.log(PHI) + 1.0
    )
...
def _external_target() -> dict:
    return {
        ...
        "название": "масса t-кварка по PDG 2024",
    }
...
CLAIMS = [
    Claim(
        name="Формула массы t-кварка против внешней оценки PDG 2024",
        source="docs/docs/math-foundations/sacred-formulas.md:140",
        ...
        claim_family="top_quark_mass",
        observable="масса t-кварка (ГэВ)",
        measurement_source="PDG 2024, таблица свойств кварков",
        uncertainty_type="both",
        expected_effect_sigma=0.526,
        resolution_sigma=1.0,
        novelty_key="quark:top_mass:external:v1",
        information_class="novelty",
        purpose="audit",
        models=["формула Trinity", "масса t-кварка PDG 2024"],
        independent_of=[
            "zeta",
            "BBLM",
            "CKM",
        ...
        notes=(
            "172.76 прочитано из строки корпуса; внешняя цель "
            "172.57 ± 0.29 ГэВ взята из таблицы PDG 2024. "
