# -*- coding: utf-8 -*-
"""Внешняя сверка формулы плотности вещества с оценкой Planck 2018.
...
def _observed() -> float:
    marker = r"| $\Omega_\text{matter}$ | 0.315 | $(8, -2, 0, 2, -2)$ | 0.31494 |"
    with open(SOURCE, encoding="utf-8") as handle:
        for line in handle:
            if marker in line:
                return 0.315
    raise AssertionError("строка корпуса с плотностью вещества не найдена")


def _reference() -> float:
    return 8.0 * 3.0**-2 * math.pi**0 * PHI**2 * math.e**-2


def _reference_alt() -> float:
    return math.exp(
        math.log(8.0) - 2.0 * math.log(3.0) + 2.0 * math.log(PHI) - 2.0
    )
...
def _external_target() -> dict:
    return {
        ...
        "название": "параметр плотности вещества по Planck 2018",
    }
...
def _multiplicity():
    ...
    fraction, expected = family.empirical_multiplicity(
        (2, -2), eps, ranges=RANGES, trials=1000, seed=20260902
    )
    return {
...
CLAIMS = [
    Claim(
        name="Формула плотности вещества против оценки Planck 2018",
        source="docs/docs/math-foundations/sacred-formulas.md:80",
        ...
        claim_family="omega_matter",
        observable="параметр плотности вещества",
        ...
        novelty_key="cosmology:omega_matter:external:v1",
        ...
        notes=(
            "0,315 прочитано из строки корпуса; внешняя цель Planck 2018 "
            "0,315 ± 0,007 взята из https://arxiv.org/abs/1807.06209. "
