# -*- coding: utf-8 -*-
"""Внешняя сверка формулы времени Планка с таблицей CODATA/NIST.

Наблюдаемое значение читается из строки корпуса, эталон формулы считается
отдельно, а внешняя оценка хранится с численной неопределённостью и URL.
...
def _observed() -> float:
    marker = r"| Planck time ($\times 10^{44}$ s) | 5.3912 |"
    with open(SOURCE, encoding="utf-8") as handle:
        for line in handle:
            if marker in line:
                return 5.3912
    raise AssertionError("строка корпуса со временем Планка не найдена")


def _reference() -> float:
    return 3.0 * 3.0**4 * math.pi**-2 * PHI * math.e**-2


def _reference_alt() -> float:
    return math.exp(
        math.log(3.0) + 4.0 * math.log(3.0) - 2.0 * math.log(math.pi)
        + math.log(PHI) - 2.0
    )
...
def _external_target() -> dict:
    return {
        "value": 5.391247,
        "uncertainty": 0.000060,
        "source": "https://physics.nist.gov/cgi-bin/cuu/Value?plkt",
        "название": "время Планка, CODATA 2022",
    }
...
def _multiplicity():
    ...
    fraction, expected = family.empirical_multiplicity(
        (-2, 1), eps, ranges=RANGES, trials=1000, seed=20260903
    )
    return {
...
CLAIMS = [
    Claim(
        name="Формула времени Планка против внешней оценки CODATA 2022",
        source="docs/docs/math-foundations/sacred-formulas.md:131",
        ...
        claim_family="planck_time",
        observable="время Планка в единицах 10^-44 секунды",
        measurement_source="CODATA 2022/NIST",
        uncertainty_type="both",
        expected_effect_sigma=3.305,
        resolution_sigma=1.0,
        novelty_key="fundamental:planck_time:external:v1",
        information_class="novelty",
        purpose="audit",
        models=["формула Trinity", "оценка CODATA 2022/NIST"],
        independent_of=[
            "zeta",
            "BBLM",
            "CKM",
        ...
        notes=(
            "5.3912 прочитано из строки корпуса; внешняя цель "
            "5.391247 ± 0.000060 в единицах 10^-44 секунды отделена URL NIST. "
            "Отклонение формулы около 3.305 сигмы; итог определяет сито, "
            "а не это поле."
        ),
    )
]
