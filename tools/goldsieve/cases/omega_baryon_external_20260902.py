# -*- coding: utf-8 -*-
"""Внешняя сверка формулы плотности барионов с оценкой Planck 2018.

Наблюдаемое значение читается из строки корпуса, формула считается отдельно,
а внешняя оценка содержит численную неопределённость и URL. Вердикт выносит
каскад золотого сита; простое подтверждение не выпускается.
"""
...
def _observed() -> float:
    marker = r"| $\Omega_\text{baryon}$ | 0.0493 | $(8, -1, -3, 3, -2)$ | 0.04931 |"
    with open(SOURCE, encoding="utf-8") as handle:
        for line in handle:
            if marker in line:
                return 0.0493
    raise AssertionError("строка корпуса с плотностью барионов не найдена")


def _reference() -> float:
    return 8.0 * 3.0**-1 * math.pi**-3 * PHI**3 * math.e**-2


def _reference_alt() -> float:
    return math.exp(
        math.log(8.0) - math.log(3.0) - 3.0 * math.log(math.pi) + 3.0 * math.log(PHI) - 2.0
    )
...
def _external_target() -> dict:
    return {
        ...
        "название": "параметр плотности барионов по Planck 2018",
    }
...
def _multiplicity():
    ...
    fraction, expected = family.empirical_multiplicity(
        (-1, -3), eps, ranges=RANGES, trials=1000, seed=20260902
    )
    return {
...
CLAIMS = [
    Claim(
        name="Формула плотности барионов против оценки Planck 2018",
        source="docs/docs/math-foundations/sacred-formulas.md:81",
        ...
        claim_family="omega_baryon",
        observable="параметр плотности барионов",
        measurement_source="Planck 2018, Cosmological Parameters",
        uncertainty_type="both",
        expected_effect_sigma=abs(_reference() - 0.315) / 0.007,
        resolution_sigma=1.0,
        novelty_key="cosmology:omega_baryon:external:v1",
        ...
        notes=(
            "0,0493 прочитано из строки корпуса; внешняя цель Planck 2018 "
            "0,0493 ± 0,0002 взята из https://arxiv.org/abs/1807.06209. "
            "Формула пересчитана отдельно; итог определяет сито, а не это поле."
        ),
    )
]
