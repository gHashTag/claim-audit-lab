...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        result = audit(path)
        # Область входа установлена отдельно от криптографического вывода:
        # этот запуск действительно прочитал настоящий журнал по пути,
        # который предъявил модуль runlog. Это не превращает хеш-цепочку или
        # наличие полей подписи в доказательство авторства.
        result["статус_входа"] = "verified-in-scope"
        result["область_входа"] = (
            "прочитан настоящий журнал запусков; число записей и число "
            "предъявленных подписей зафиксированы отдельно"
        )
    OUT.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n",
                   encoding="utf-8")
    if result.get("статус_входа") == "verified-in-scope":
        print("статус входа: verified-in-scope; настоящий журнал прочитан")
