...
def inspect(document: object, source: str) -> dict:
    ...
    missing_ids = [
        item for item in records
        if not isinstance(item["идентификатор_общего_ансамбля"], str)
        or not item["идентификатор_общего_ансамбля"].strip()
    ]
    if missing_ids:
        return {
            "статус": "not-evaluated",
            "причина": (
                "идентификатор общего ансамбля не предъявлен для каждой "
                "записи; одного числа общего_M_eff недостаточно"
            ),
            "источник_наблюдения": source,
            "записей_С20": len(records),
            "наблюдения": records,
...
def selftest() -> int:
    ...
    without_id = inspect(
        [missing_identifier], "фикстура/без-идентификатора-ансамбля.json")
    check("одного общего M_eff без идентификатора недостаточно",
          without_id["статус"] == "not-evaluated"
          and "идентификатор общего ансамбля" in without_id["причина"])
