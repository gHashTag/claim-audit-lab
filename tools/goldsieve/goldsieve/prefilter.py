...
def decide(target: float, *, uncertainty: float | None = None,
           ...
           values: list[float] | None = None) -> Decision:
...
def selftest() -> int:
    ...
    # --- вырожденные входы ---------------------------------------------------
    check("нулевая цель даёт RUN", decide(0.0, values=vals).verdict == RUN)
    check("нечисло даёт RUN", decide(float("nan"), values=vals).verdict == RUN)
    ...
    coarse = decide(16.66, uncertainty=0.59, values=vals)
    fine = decide(16.66, uncertainty=1e-9, values=vals)
    ...
    homo = decide(878.4, uncertainty=0.5,
                  ...
                  ranges=_t.ACTUAL_RANGES, values=av)
