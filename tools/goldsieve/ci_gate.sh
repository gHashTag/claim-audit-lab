...
if [ -z "$BASE_PY" ]; then
    ...
    # загрузки; переход ../ или симлинк наружу может подменить исполняемый
    # модуль регресса. Сторож требует путь внутри рабочей копии.
    step "чувствительность границы путей кейсов" "$BASE_PY" registry_case_path_guard.py --selftest
    step "граница путей кейсов реестра" "$BASE_PY" registry_case_path_guard.py
    # Новый риск: С13 может увидеть объявленный пропуск, но опечатанный ключ
    # или пустое объяснение останется мёртвым полем до срабатывания сита.
    # AST-проверка не исполняет кейсы; динамические словари не объявляются
    # покрытыми.
    step "чувствительность ключей skip_reasons" "$BASE_PY" skip_reason_contract_guard.py --selftest
    step "контракт ключей skip_reasons" "$BASE_PY" skip_reason_contract_guard.py
    ...
    # Повтор одного пути в одном запуске не является двумя независимыми
    # артефактами: иначе происхождение можно посчитать дважды.
    step "однозначность списка артефактов" "$BASE_PY" artifact_uniqueness_guard.py
    step "чувствительность однозначности артефактов" "$BASE_PY" artifact_uniqueness_guard.py --selftest
    ...
    # Тик 394: буквальное совпадение AST observed/reference внутри Claim —
    # отдельный риск рецепта. Оно оставляется not-evaluated и не превращается
    # в научный вердикт; косвенные цепочки остаются в identity detector.
    step "чувствительность прямого повтора рецепта" "$BASE_PY" recipe_tautology_guard.py --selftest
    step "аудит прямого повтора рецепта" "$BASE_PY" recipe_tautology_guard.py
    ...
    # Тик 379: свободное описание после пути source не должно становиться
    # неразрешённым входом инкрементального регресса.
    step "чувствительность фрагментов источника" "$BASE_PY" source_fragment_guard.py --selftest
    step "фрагменты источника реестра" "$BASE_PY" source_fragment_guard.py
    # Тик 381: существующий source может быть символической ссылкой наружу.
    # Разрешимость такого входа не доказывает, что наблюдаемое принадлежит
    ...
    # Тик 398: отдельный машинный вопрос для независимого вывода
    # коэффициентов. Отсутствие аналитического источника с формулой и номером
    # уравнения сохраняется как analytic_source_absent, а не превращается в
    # доказанный коэффициент или научную находку.
    step "чувствительность вывода коэффициентов BBLM" "$BASE_PY" bblm_coefficient_rederivation_guard.py --selftest
    step "машинный вопрос вывода коэффициентов BBLM" "$BASE_PY" bblm_coefficient_rederivation_guard.py
    ...
    # Тик 378: опечатка в верхнеуровневом статусе не должна выглядеть новым
    # результатом; отсутствие статуса остаётся not-evaluated.
    step "словарь статусов машинных отчётов" "$BASE_PY" status_domain_guard.py
    step "чувствительность словаря статусов" "$BASE_PY" status_domain_guard.py --selftest
