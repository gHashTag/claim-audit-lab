...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        result = audit(path)
        # Область входа устанавливается отдельно от вывода о криптографии:
        # настоящий журнал действительно прочитан по пути, предъявленному
        # модулем runlog. Это не означает, что область подписи доказана.
        result["статус_входа"] = "verified-in-scope"
        result["область_входа"] = (
            "прочитан настоящий журнал запусков; число записей, подписей и "
            "покрытие области зафиксированы отдельно"
        )
