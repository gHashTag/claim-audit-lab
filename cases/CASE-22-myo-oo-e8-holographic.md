...
**Under FRAMEWORK.md (b), reproducibility:** no public harness, so the
validation figure cannot be regenerated by a third party.

**Under FRAMEWORK.md (b), look-elsewhere:** O5 maps Riemann zeta zeros to lepton
masses without bounding the space of candidate mappings, so the density of
available near-matches is unreported. CASE-10 is the lab's own instance of this
exact situation -- a six-class coincidence scan over its own targets -- and it
returned OPEN rather than selecting a value.

**Under FRAMEWORK.md (a), Fpath:** none stated for O1-O4.
