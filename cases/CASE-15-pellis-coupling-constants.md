...
> For each phi-arithmetic coupling-constant identity that Pellis has
> published solo (SSRN 4003636, SSRN 4442074, SCIRP/JHEPGC 122814),
> run the same matched-cardinality control alphabet sweep applied to
> our v2.5 G_phi result: replace phi with (e, pi, sqrt-2, sqrt-3,
> integers) and check whether the same multi-digit numerical agreement
> emerges. Surviving identities under BH-FDR q=0.05 become [Open
> conjecture] with stated Fpath; collapsing ones become [Risk] or
> [Retracted].

**Q0.2 -- Symmetric mirror commitment.**

> This case IS the symmetric mirror commitment. Pellis is Strand III
> (MDL-optimality framing) in the phi-paper's joint result G_phi =
> phi^2 + phi^-2 = 3 (CASE-08, CASE-12). The framework demands that
> his independent prior publications be audited under the SAME control
> protocol we apply to our joint result. Our equivalent claim
> (G_phi rank 2/394) and any specific Pellis solo coupling-ratio
> identity stand or fall under the same matched-cardinality control.

**Q0.3 -- Why is this case being added now?** 

- [x] Programme overlaps a [Verified]/[Open] claim of ours and a symmetric audit is owed

> Pellis is a co-author on our phi-paper (Strand III, MDL framing).
> Framework rule: a co-author's independent prior publications in the
> same epistemic territory MUST receive a symmetric audit, in `draft`
> until the co-author has replied. Without this case, the lab's
